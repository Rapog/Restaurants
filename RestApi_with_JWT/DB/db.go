...
func (Places) GetPlaces(limit int, offset int) ([]Place, int, error) {
	...
	//countPlaces = esStr.Hits.Total.Value
...
func (Places) GetPlacesGeo(latStr string, lonStr string) []Place {
	es, err := elasticsearch.NewDefaultClient()
	if err != nil {
		log.Fatalf("Creating elastic client error: %s", err)
	}
	var query string
	query = "{\"size\": 3, \"sort\": [{\"_geo_distance\": {\"location\": {\"lat\": " + latStr + ",\"lon\": " + lonStr + "}," +
		"\"order\": \"asc\",\"unit\": \"km\",\"mode\": \"min\",\"distance_type\": \"arc\",\"ignore_unmapped\": true}}]}"
	res, err := es.Search(
		es.Search.WithBody(strings.NewReader(query)),
		es.Search.WithPretty(),
	)
